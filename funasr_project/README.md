...
版本：`v0.3.3-competition-compliance`
...
指令识别默认关闭标点恢复模型，以减少内存占用；DataSetA 本地 CER 默认按原始字符计算。`--local-normalize` 仅用于旧结果调试，最终以主办方 scorer 为准。
...
| --- | ---: |
| 正样本 / 负样本 | 1,364 / 474 |
| 本地原始字符 CER | 119.17%（9,515 个参考字） |
| 正样本接收率 | 69.35% |
| RR | 91.14%（432 / 474） |
| 端到端耗时 | 618.4 s，约 0.336 s/条 |

此结果不同于 AISHELL 纯净开发集的 1.19% CER，也不同于公开混叠 smoke test 的 25.56% CER：前两者分别衡量基础转写能力与净化增益；本表只衡量 DataSetA 的阶段端到端效果，包含声纹门控的误拒绝损失，不代表 B 集最终成绩。CER 可以超过 100%，表示插入、替换和删除错误总数超过参考字数。v0.3.2 的 52.87% 使用本项目文本归一化，不能与本表的原始字符口径直接比较。
