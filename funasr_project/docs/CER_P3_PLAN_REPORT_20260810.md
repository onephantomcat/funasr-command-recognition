...
19:15 状态检查：修正版 B1 正在 step `1700/20000`，GPU `62%`、显存 `2271 MiB`。step `1538` 的一次非有限梯度已正确跳过 optimizer 更新，GradScaler 从 `512` 回退到 `256` 后训练继续；尚未产生 step 2000 的首轮 dev 指标，也没有最终 verdict。由于现有必过项仍要求训练期间所有梯度有限，本次最终工程 verdict 已预计为 `FAIL`；继续运行只为取得 dev/restore 证据，不放行 B3，不修改配置或放宽判定。
