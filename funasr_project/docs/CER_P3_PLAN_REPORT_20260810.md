...
19:15 状态检查：修正版 B1 正在 step `1700/20000`，GPU `62%`、显存 `2271 MiB`。step `1538` 的一次非有限梯度已正确跳过 optimizer 更新，GradScaler 从 `512` 回退到 `256` 后训练继续；尚未产生 step 2000 的首轮 dev 指标，也没有最终 verdict。未修改配置、未启动 B3。
