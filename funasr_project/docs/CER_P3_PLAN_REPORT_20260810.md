...
21:43 状态检查：修正版 B1 已完成 step 2000 与 step 4000 两轮各 10,000 条 dev 全量评估，训练恢复到 step `4125/20000`；对应 checkpoint 均存在。step 2000 为 `dev_loss=-1.8249511459 / dev_sisdr=3.1341933973 dB`，step 4000 改善到 `dev_loss=-2.5572967597 / dev_sisdr=3.8020851040 dB`。检查瞬间 GPU `31%`、显存 `2271 MiB`，主进程和两个 DataLoader 子进程正常。累计仍只有 step `1538` 的一次非有限梯度，optimizer 更新已正确跳过，GradScaler 从 `512` 回退到 `256` 后未再出现同类记录；最终 verdict 尚未生成。质量趋势改善，但现有必过项仍要求训练期间所有梯度有限，本次最终工程 verdict 仍预计为 `FAIL`；继续运行只为取得最终 dev/restore 证据，不放行 B3，不修改配置或放宽判定。
