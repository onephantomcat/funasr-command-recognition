...
19:34 状态检查：修正版 B1 已到 step `2000/20000` 并保存 `checkpoint_step2000.pt`，正在执行 10,000 条 dev 全量评估，日志在 19:31 已记录至少 `1000/10000` 条。检查瞬间 GPU `0%`、显存 `2271 MiB`，主进程和两个 DataLoader 子进程仍在；这是 dev 数据处理中的瞬时状态，不是训练退出。累计仍只有 step `1538` 的一次非有限梯度，optimizer 更新已正确跳过，GradScaler 从 `512` 回退到 `256` 后未再出现同类记录；首轮 dev 指标和最终 verdict 尚未生成。由于现有必过项仍要求训练期间所有梯度有限，本次最终工程 verdict 已预计为 `FAIL`；继续运行只为取得 dev/restore 证据，不放行 B3，不修改配置或放宽判定。
