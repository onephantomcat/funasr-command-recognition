...
运行：
  .\\.venv-p2tse\\Scripts\\python.exe p2_tools/train_overfit_debug.py --device auto
"""
...
def frame_activity(activity, win_length, hop_length, ratio_thresh):
    ...
    """
    a = torch.from_numpy(activity).float().unsqueeze(0).unsqueeze(0)  # [1,1,T]
    ...
    return (frame_ratio.squeeze(0).squeeze(0) > ratio_thresh).float()  # [Fr]
...
def main():
    ...
    ap.add_argument("--config", default=str(ROOT / "configs" / "p2" / "tse_overfit_debug.yaml"))
    ...
    out_dir = Path(args.out) if args.out else ROOT / "artifacts" / "experiments" / f"DEBUG_OVERFIT_seed{seed}"
    if out_dir.exists():
    ...
    # 数据留档（8 条 mixture+target 的 sha256）
    with open(out_dir / "data.sha256", "w", encoding="utf-8") as f:
        f.write(f"{sha256_file(args.manifest)}  manifest.jsonl\n")
        for e in batch["entries"]:
            f.write(f"{sha256_file(ROOT / e['mixture'])}  {Path(e['mixture']).name}\n")
            f.write(f"{sha256_file(ROOT / e['target'])}  {Path(e['target']).name}\n")
    ...
    model.train()
    for step in range(steps + 1):  # step0 = 未训练的初始前向
        t0 = time.time()
        if step > 0:
