...
运行示例：
  ...
  .\\.venv-p2tse\\Scripts\\python.exe p2_tools/train_b1_trial.py --debug_data --max_steps 10

  # 正式 500 步试车（需 P1 v2_b1 交付）
  .\\.venv-p2tse\\Scripts\\python.exe p2_tools/train_b1_trial.py --device auto

  # 从 checkpoint 续训
  .\\.venv-p2tse\\Scripts\\python.exe p2_tools/train_b1_trial.py --resume artifacts/experiments/B1_TRIAL_seed20260723/checkpoint_step100.pt
"""
...
from src.tse.losses import si_sdr  # noqa: E402
from train_overfit_debug import (  # noqa: E402
    bootstrap_embedding,
    compute_losses,
    frame_activity,
    sha256_file,
...
DEBUG_MANIFEST = ROOT / "artifacts" / "p2" / "debug_mixtures_v0" / "manifest.jsonl"
...
class B1Dataset(data.Dataset):
    ...
    def __init__(self, manifest_path, cfg, seed=None):
        self.entries = []
        ...
        # 独立 generator 控制随机截取起点，不污染全局 seed
    ...
    def __getitem__(self, idx):
        e = self.entries[idx]
        mix, sr = sf.read(str(ROOT / e["mixture"]), dtype="float32")
        tgt, _ = sf.read(str(ROOT / e["target"]), dtype="float32")
        itr, _ = sf.read(str(ROOT / e["interferer"]), dtype="float32")
        ...
        return {
            ...
            "emb": bootstrap_embedding(e["enrollment"], self.emb_dim),
        }
...
def main():
    ...
    ap.add_argument("--config", default=str(ROOT / "configs" / "p2" / "tse_b1_trial.yaml"))
    ...
    # AMP 仅在 CUDA 上启用；CPU 强制关
    use_amp = bool(cfg["amp"]) and device.type == "cuda"

    # 输出目录
    tag = cfg.get("tag", "B1_TRIAL")
    out_dir = Path(args.out) if args.out else ROOT / "artifacts" / "experiments" / tag
    if args.resume:
    ...
    LOG.info("config=%s device=%s seed=%d amp=%s", args.config, device, seed, use_amp)
    ...
    # ---- batch_size 适配（debug_data 时样本可能不足）----
    ...
    # ---- Dataset + DataLoader ----
    train_ds = B1Dataset(train_manifest, cfg, seed=seed)
    dev_ds = B1Dataset(dev_manifest, cfg, seed=seed + 1)
    ...
    # torch 2.7+ API：torch.amp.GradScaler('cuda', ...)（旧 torch.cuda.amp.GradScaler 已弃用）
    ...
    # 空跑测试时 total_steps 可能 < save_every，自动降低以保证至少存一次 checkpoint
    ...
    for step in range(start_step, total_steps + 1):
        ...
        # 基线 SI-SDR(mixture) —— 首步记录一次，用于 SI-SDRi
    ...
    # loss 趋势：步数充足(≥200)时取首/末各 100 步；不足时取前/后半
    if total_steps >= 200:
    ...
    # 必过项（任一 FAIL 则总体 FAIL）；loss_decreasing 为建议项，单独报告不否决
    verdicts = {
        "no_nan": nan_steps == 0,
        ...
        "loss_decreasing": bool(loss_decreasing),  # 建议项
