...
运行：
  python p2_tools/build_debug_mixtures.py --seed 20260725
"""
...
ACT_THRESH = 1e-4  # activity 判定阈值（|target| 超过即视为活动）


def read_wav(path, n=N, rng=None):
    """读 wav，截断/补零到 n 采样；确定性。"""
    wav, sr = sf.read(str(path), dtype="float32")
    assert sr == SR, f"{path} 采样率 {sr} != {SR}"
    if wav.ndim > 1:
        wav = wav[:, 0]
    if len(wav) >= n:
        # 确定性起点：用 rng 选偏移（同一 seed 可复现）
...
def place(full_n, seg, start):
    """把 seg 放置到 full_n 长度的 start 位置，其余为零。"""
...
def scale_to_sir(target, interferer, sir_db, eps=1e-8):
    """按各自活动区 RMS 调 interferer 增益，使 SIR 达标。"""
...
def build_one(rng, spk_idx, config):
    ...
    if config == "partial25":
        ...
        enroll_spk = swap_spk  # enrollment-swap：注册音频来自他人
    else:
        raise ValueError(config)

    interferer = scale_to_sir(target, interferer, sir)

    # 防削波：整体增益同时作用于两路 stem，保持 mixture = t + i 严格成立
    ...
    return {
        "id": uid,
        "mixture": str(paths["mixture"].relative_to(ROOT)),
        "target": str(paths["target"].relative_to(ROOT)),
        "interferer": str(paths["interferer"].relative_to(ROOT)),
        "activity": str(paths["activity"].relative_to(ROOT)),
...
def main():
    ...
    meta = {
        ...
        "command": f"python p2_tools/build_debug_mixtures.py --seed {args.seed}",
        "seed": args.seed,
