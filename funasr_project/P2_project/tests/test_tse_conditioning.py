# -*- coding: utf-8 -*-
"""P2-08 注册条件验证（五条件测试）。v2 分支版。

目的：证明 enrollment 条件（FiLM 注入）确实影响 TSE 输出，而不是摆设；
同时验证条件路径不会凭空制造能量。

运行：
  python tests/test_tse_conditioning.py --device auto
...
REPORT_PATH = P2_ROOT / "reports" / "smoke" / "tse_conditioning.json"
...
def main():
    ...
    parser.add_argument("--config", default=str(P2_ROOT / "configs" / "tse_smoke.yaml"))
    ...
    with torch.no_grad():
        s1, r1, _ = model(x, e_a)
        s1_b, r1_b, _ = model(x, e_a)
        s2, r2, _ = model(x, e_b)
        s0, r0, _ = model(x, e_zero)
        s_sh, r_sh, _ = model(x, e_shuffle)
        x_zero = torch.zeros_like(x)
        sz, rz, _ = model(x_zero, e_a)
