# -*- coding: utf-8 -*-
"""P2-06 双输出 TSE smoke 的 pytest 薄包装（教练要求入口：pytest tests/p2 -v）。

断言口径与 tests/p2/smoke_tse_random.py 一致；P2-08 条件验证见
...
CONFIG_PATH = ROOT / "configs" / "p2" / "tse_smoke.yaml"
...
def test_backward_and_grads(cfg, model):
    ...
    s, r, p = model(x, e)
    # act_bce 项覆盖 act_head 梯度（帧活动度取全 1，仅验证梯度通路）
